...
function opensua() {
  if (selectedNhapKho) {
    bang_them_nhap_kho.style.display = "block";
    document.querySelector("input[name='ma_san_pham']").value =
      selectedNhapKho.ma_san_pham;
    document.querySelector("input[name='so_luong']").value =
      selectedNhapKho.so_luong;
    document.querySelector("input[name='ngay_nhap']").value = convertDateFormat(
      selectedNhapKho.ngay_nhap
    );
    document.querySelector("input[name='dien_giai']").value =
      selectedNhapKho.dien_giai;
    if (document.querySelector("input[name='ma_nhap_hidden']")) {
      document.querySelector("input[name='ma_nhap_hidden']").value =
        selectedNhapKho.ma_nhap;
    }
  } else {
...
function start() {
  Promise.all([
    fetch("https://localhost:7103/api/QuanLyNhapKho/getAllQuanLyNhapKho").then(
      (res) => res.json()
    ),
    fetch("https://localhost:7103/api/QuanLySanPham/getAllQuanLySanPham").then(
      (res) => res.json()
    ),
...
function xuLyTaoMoi() {
  ...
  if (taoMoiNhapKho) {
    taoMoiNhapKho.addEventListener("click", function (event) {
      ...
      event.preventDefault();
      var maSanPham = document
        .querySelector("input[name='ma_san_pham']")
        .value.trim();
      var soLuongInput = document
        .querySelector("input[name='so_luong']")
        .value.trim();
      var ngayNhapRaw = document
        .querySelector("input[name='ngay_nhap']")
        .value.trim();
      var dienGiai = document
        .querySelector("input[name='dien_giai']")
        .value.trim();
...
function deleteNhapKho(maNhap) {
  fetch(
    `https://localhost:7103/api/QuanLyNhapKho/deleteQuanLyNhapKho?ma_nhap=${maNhap}`,
    {
      method: "DELETE",
    }
  )
...
function openxoa() {
  if (selectedNhapKho) {
    if (
      confirm(
        `Bạn có chắc chắn muốn xóa phiếu nhập ${selectedNhapKho.ma_nhap} không?`
      )
    ) {
